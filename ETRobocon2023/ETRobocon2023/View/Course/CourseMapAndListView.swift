        ...
        HStack {
            GeometryReader { geometry in
                CourseView(isAnimating: $isAnimating, segment: $selection)
                    ...
                    .frame(
                        width: min(geometry.size.height * (LCourseViewSize.frameWidth / LCourseViewSize.frameHeight), geometry.size.width),
                        height: min(geometry.size.height, geometry.size.width * (LCourseViewSize.frameHeight / LCourseViewSize.frameWidth))
                    )
            }
